...
with chosen_topic as (
  select id from public.topics where subject = 'Food Science and Nutrition' order by case when level = 'advanced' then 0 else 1 end, title limit 1
), existing as (
  select id from public.course_documents where title = 'CAMEROON GCE ADVANCED LEVEL FOOD SCIENCE AND NUTRITION P2 SET 3' limit 1
)
insert into public.course_documents (
  ...
  'Food Science and Nutrition', 'CAMEROON GCE ADVANCED LEVEL FOOD SCIENCE AND NUTRITION P2 SET 3', 'english', 'advanced',
  array['form_3', 'form_4', 'form_5', 'lower_sixth', 'upper_sixth']::text[],
  array['general', 'science', 'arts', 'commercial', 'technical', 'a_science', 'a_arts', 'a_commercial', 'a_technical']::text[],
  'published', '# CAMEROON GCE ADVANCED LEVEL FOOD SCIENCE AND NUTRITION P2 SET 3
...
with chosen_topic as (
  select id from public.topics where subject = 'Logic' order by case when level = 'ordinary' then 0 else 1 end, title limit 1
), existing as (
  select id from public.course_documents where title = 'CAMEROON GCE ORDINARY LEVEL LOGIC P2 SET 1' limit 1
)
insert into public.course_documents (
  id, topic_id, subject, title, language, level, class_levels, series, status, markdown_content, created_by
) values (
  coalesce((select id from existing), gen_random_uuid()),
  (select id from chosen_topic),
  'Logic', 'CAMEROON GCE ORDINARY LEVEL LOGIC P2 SET 1', 'english', 'ordinary',
  array['form_3', 'form_4', 'form_5', 'lower_sixth', 'upper_sixth']::text[],
  array['general', 'science', 'arts', 'commercial', 'technical', 'a_science', 'a_arts', 'a_commercial', 'a_technical']::text[],
  'published', '# CAMEROON GCE ORDINARY LEVEL LOGIC P2 SET 1
