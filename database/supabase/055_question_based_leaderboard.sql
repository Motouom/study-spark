...
topic_totals as (
  select
    progress.user_id,
    count(progress.id) filter (where progress.status = 'understood')::integer as topics_understood,
    count(progress.id) filter (where progress.status = 'review')::integer as topics_review
  from public.topic_understanding_progress progress
  group by progress.user_id
),
learner_rows as (
  select
    ...
    profile.series,
    case
      when profile.education_system in ('gce', 'francophone') then profile.education_system
      when profile.class_level in ('sixieme','cinquieme','quatrieme','troisieme','seconde','premiere','terminale') then 'francophone'
      else 'gce'
    end as education_system,
    ...
    coalesce(topic_totals.topics_understood, 0)::integer as topics_understood,
    coalesce(topic_totals.topics_review, 0)::integer as topics_review,
  ...
  left join topic_totals on topic_totals.user_id = profile.user_id
)
select
  ...
  learner_rows.series,
  learner_rows.education_system,
  learner_rows.papers_read,
  learner_rows.papers_completed,
  learner_rows.study_days,
  learner_rows.useful_checkpoints,
  (
    learner_rows.questions_passed * 3 +
    learner_rows.topics_understood * 5 +
    learner_rows.questions_failed * 1 +
    learner_rows.topics_review * 2
  )::integer as score,
...
order by
  ...
  learner_rows.topics_understood desc,
