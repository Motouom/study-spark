...
import { BookOpen, Clock, GraduationCap, ListChecks, PlayCircle } from "lucide-react";
          ...
          {loaded && courses.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {courses.map((course) => {
                ...
                const units = [
                  ...new Set(
                    [...course.markdownContent.matchAll(/^## (.+)$/gm)].map((match) =>
                      match[1].trim(),
                    ),
                  ),
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                      <CourseMetric icon={BookOpen} label="Units" value={String(units.length)} />
                      <CourseMetric
                        icon={Clock}
                        label="Time studied"
                    ...
                    <div className="mt-auto pt-4">
                      <Button asChild className="w-full" size="sm">
                        <Link to="/course/$documentId" params={{ documentId: course.id }}>
                          {courseProgress.length > 0 ? "Continue course" : "Start course"}
                        </Link>
                      </Button>
                      {passed > 0 && (
                        <p className="mt-2 text-center text-xs text-muted-foreground">
                          {passed} question{passed === 1 ? "" : "s"} passed in this course
                        </p>
                      )}
                    </div>
                  </article>
