  ...
  return (
    <div className="min-h-[calc(100dvh-7.5rem)] bg-surface px-4 py-6 md:min-h-[calc(100dvh-3rem)] md:py-10">
      <div className="mx-auto max-w-xl">
        ...
        <div
          key={step}
          className="animate-fade-up rounded-2xl border border-border bg-card p-6 md:p-8"
        >
          {step === 0 && (
            <div>
              ...
              <label className="mt-6 block text-sm font-medium">{t("onboarding.nameLabel")}</label>
              <Input
                autoFocus
                value={name}
            ...
            <div>
              ...
              {(browserLocation.error || locationVerified) && (
                <p className="mt-2 text-xs text-muted-foreground">
                  {locationVerified ? t("onboarding.locationVerifiedHint") : browserLocation.error}
                </p>
              )}
            </div>
          )}
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        setSeries(item.id);
                        setSubjects((current) =>
                          current.filter((subject) => subjectsForSeries(item.id).includes(subject)),
                        );
                      }}
                      className={`flex items-center justify-between rounded-lg border p-4 text-left transition-colors ${
                        sel
