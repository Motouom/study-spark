...
const emptyDocument = (): CourseDocumentDraft => ({
  topicId: "",
  subject: "Mathematics",
  title: "",
  language: "english",
...
function QuestionsAdmin() {
  ...
  async function submitDocument(event: React.FormEvent) {
    ...
    try {
      ...
      setMode("list");
      setMessage("Markdown document published without emojis.");
    } catch (err) {
      setMessage(err instanceof Error ? err.message : "Markdown document could not be saved.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function formatDocumentWithAi() {
    ...
    try {
      ...
      setMessage(
        result.source === "ai"
          ? "AI formatted the paper. Review it before publishing."
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Upload, update, publish, and retire protected structural question papers.
          </p>
        </div>
      ...
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
        <StatCard label="Courses" value={String(topics.length)} />
        <StatCard label="Papers" value={String(documents.length)} />
        <StatCard
          label="Published papers"
          value={String(documents.filter((item) => item.status === "published").length)}
          tone="success"
        />
      </div>
      ...
      {mode === "document" && (
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-medium">Upload Markdown course document</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                Paste or upload a complete formatted .md question bank. Emojis are removed before
                saving.
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <Field label="Title">
              <Input
                ...
                className="uppercase"
                placeholder="Advanced Level Mathematics Paper 2 Set 2"
              />
            </Field>
            <Field label="Subject">
              <Select
          ...
          <div className="mt-3 flex flex-col gap-3 rounded-lg border border-border bg-secondary/30 p-3 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              ...
              {ai.loading === "format-paper" ? "Formatting..." : "AI format paper"}
            </Button>
          </div>
          ...
          <div className="mt-4 flex justify-end gap-2">
            ...
            >
              ...
              {saving ? "Publishing..." : "Publish markdown document"}
            </Button>
          </div>
        </form>
      )}
  ...
  return (
    <section className="mt-6 rounded-xl border border-border bg-card">
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div>
          <h2 className="text-sm font-medium">Markdown course documents</h2>
          <p className="text-xs text-muted-foreground">Full formatted in-app documents.</p>
        </div>
        <Badge variant="secondary">{documents.length}</Badge>
      </div>
      {documents.length === 0 ? (
        <div className="px-4 py-8 text-center text-sm text-muted-foreground">
          No Markdown documents uploaded yet.
        </div>
      ) : (
            ...
            <div key={document.id} className="flex items-center justify-between gap-4 px-4 py-3">
              ...
              <div className="flex shrink-0 items-center gap-1">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (window.confirm("Delete this paper permanently? This cannot be undone.")) {
