-- Fix false profile-lock failures when an existing blank city is stored as NULL
-- but the settings form submits it as an empty string.
...
declare
  ...
  normalized_city text := nullif(trim(coalesce(profile_city, '')), '');
begin
  ...
  if existing_profile.user_id is not null and has_progress and (
    ...
    nullif(trim(coalesce(existing_profile.city, '')), '') is distinct from normalized_city or
