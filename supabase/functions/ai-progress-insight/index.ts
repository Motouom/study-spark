...
import { aiConfigured, fallbackInsight, generateAiText } from "../_shared/ai.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const [{ data: profile }, { data: sessions }, { data: checkpoints }, { data: documents }] =
      await Promise.all([
        supabase
    ...
    const averageDepth =
      rows.length > 0
        ? Math.round(
            rows.reduce((sum, item) => sum + Number(item.max_scroll_percent ?? 0), 0) / rows.length,
          )
        : 0;
    ...
    const fallback = fallbackInsight({
      ...
      bookmarkCount: checkpointRows.filter((item) => item.checkpoint_type === "bookmark").length,
      averageDepth,
      totalDuration:
    ...
    if (!aiConfigured()) return json({ insight: fallback, source: "fallback" });
    ...
    try {
      ...
      return json({ insight: fallback, source: "fallback" });
    }
  } catch (error) {
